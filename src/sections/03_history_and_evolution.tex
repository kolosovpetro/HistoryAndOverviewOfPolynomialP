...
by Issac Newton's fundamental work on the topic that is nowadays known as foundation of classical interpolation theory (citations).
...
\begin{table}[H]
    ...
    \label{tab:fig_1}
\end{table}
Therefore, we have reached our base case, the case $n^3$.
...
showing the cases for polynomials $n^5, \; n^7$ and $n^9$.
...
Therefore, the relation~\eqref{eq:cube_identity} we got previously by interpolation can be generalized for
...
In more details, it is discussed separately in~\cite{kolosov2022106, kolosov2023polynomial}.
