...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
Hence, we have successfully reached interpolation of the polynomial $n^3$
\begin{equation}
...
why the above identity is true arranging the terms $30k^2(n-k)^2 + 1$ over $0 \leq k \leq n$ as tabular.
See OEIS sequence~\cite{kolosov2018fifth}.

Now we can see that the relation~\eqref{eq:cube_identity} we got previously via interpolation of the polynomial $n^3$
can be generalized for all odd-powers $2m+1$ by constructing and solving certain system of linear equations,
and its generalized form appears to be
\begin{equation}
...
there must be a function that generates the set of real coefficients $\coeffA{m}{r}$ for each fixed $m$, I thought.
...
in OEIS~\cite{kolosov2018numerator,kolosov2018denominator}, respectively.
It is as well interesting to notice that row sums of the $\coeffA{m}{r}$ give powers of $2$
\begin{equation*}
    \sum_{r=0}^{m} \coeffA{m}{r} = 2^{2m+1} - 1
\end{equation*}
Therefore, we have successfully generalized previously obtained equation~\eqref{eq:cube_identity} for each
natural $m\geq 0$
\begin{thm}
...
Introducing the parameter $b$ to the upper summation bound to~\eqref{eq:odd-power-theorem},
