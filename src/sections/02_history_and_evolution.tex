...
However, constructing and solving systems of linear equations each time for every odd-power $2m+1$ requires a huge effort,
...
Applying both Binomial theorem and Faulhaber's formula
...
\end{equation*}
Therefore, we have successfully generalized obtained previously cube identity~\eqref{eq:cube_identity}
for each non-negative odd-power $2m+1$
\begin{thm}
